...
public extension JSONEncoder {
    ...
    func encodeWithJson<T:Encodable>(_ objcet: T, addKeyPath keypath: String?) throws -> Any? {
        guard let encodedData = try encode(objcet, addKeyPath: keypath) else { return nil }
        
        guard let jsonObject = try? JSONSerialization.jsonObject(with: encodedData, options: .allowFragments) else { return nil }
        return jsonObject
    }
}
...
public extension PropertyListEncoder {
    ...
    func encodeWithJson<T:Encodable>(_ objcet: T, addKeyPath keypath: String?) throws -> Any? {
        ...
        outputFormat = .xml
        guard let jsonObject = try? PropertyListSerialization.propertyList(from: encodedData, options: .mutableContainers, format: nil) else { return nil }
        return jsonObject
    }
}
